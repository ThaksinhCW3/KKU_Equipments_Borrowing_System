    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="flex flex-col gap-3">
            ...
            @if (Route::has('password.request'))
                <a class="text-sm text-center text-black hover:text-indigo-700" href="{{ route('password.request') }}">
                    {{ __('ลืมรหัสผ่าน?') }}
                </a>
            @endif

            <a href="{{ route('register') }}" class="text-sm text-center text-indigo-600 hover:text-indigo-800">
