...
class BorrowRequestController extends Controller
{
    ...
    public function approve(Request $req, $req_id)
    {
        ...
        Log::create([
            ...
            'description' => "อนุมัติคำขอยืม: {$borrowRequest->req_id}",
    ...
    public function reject(Request $req, $req_id)
    {
        ...
        Log::create([
            ...
            'description' => "ปฏิเสธคำขอยืม: {$request->req_id}",
            'module' => 'borrow_request',
            'severity' => 'warning',
            'old_values' => $oldData,
            'new_values' => $newData,
...
public function update(Request $req, $req_id)
{
    ...
    if ($borrowRequest->status === 'approved') {
        ...
        Log::create([
            ...
            'target_id' => $borrowRequest->id,
            'target_name' => $borrowRequest->req_id,
            'description' => "เช็คเอาท์อุปกรณ์: {$borrowRequest->req_id} - อุปกรณ์: " . ($borrowRequest->equipment->name ?? 'N/A'),
            'module' => 'borrow_request',
            'severity' => 'info',
            'ip_address' => $req->ip(),
            'user_agent' => $req->userAgent(),
        ...
        Log::create([
            ...
            'target_id' => $borrowRequest->id,
            'target_name' => $borrowRequest->req_id,
            'description' => "เช็คอินอุปกรณ์: {$borrowRequest->req_id} - อุปกรณ์: " . ($borrowRequest->equipment->name ?? 'N/A'),
            'module' => 'borrow_request',
            'severity' => 'info',
            'ip_address' => $req->ip(),
            'user_agent' => $req->userAgent(),
